...
void branchAndBoundHeuristic(int graph[MAX][MAX], int h[], int n, int start, int goal) {
    Node pq[MAX * MAX];
    int pqSize = 0;
    ...
    while (pqSize > 0) {
        ...
        pqSize--;
        for (int i = bestIndex; i < pqSize; i++) pq[i] = pq[i + 1];
...
int main() {
    ...
    branchAndBoundHeuristic(graph, h, n, start, goal);
